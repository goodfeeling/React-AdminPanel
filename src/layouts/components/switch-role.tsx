...
const SwitchModal = ({ show, onCancel }: SwitchModalProps) => {
	...
	const { roles, current_role_id, current_role_name } = useUserInfo();
	...
	// 初始化默认选中 current_role_id
	useEffect(() => {
		if (current_role_id) {
			setSelectedKeys([String(current_role_id)]);
		}
	}, [current_role_id]);
	const handleCancel = () => {
		setOpen(false);
	...
	const onSelect: TreeProps["onSelect"] = async (keys) => {
		...
		try {
			...
			// 接口调用成功后更新 store 中的 current_role_id
	...
	return (
		<>
			<Modal open={open} title={`当前角色选中：${current_role_name}`} onCancel={handleCancel} footer={null}>
				<Tree
					showLine
